...
function buildTestService() {
	...
	service.define({
		name: 'asyncSum',
		params: [{ name: 'a', type: '*' }, { name: 'b', type: '*' }],
		returns: '*',
	});
	service.define(
		{
...
function startServer(done, options = {}) {
	...
	const serveMetadata =
		typeof options.serveMetadata === 'undefined' ? true : options.serveMetadata;
...
['HTTP', 'WebSocket', 'Socket.IO'].forEach(transportName =>
	describe(`node.js proxy - ${transportName} transport`, function() {
		...
		function expectProxyEvent(proxy, event, errorMessage, timeout = 1000) {
			return new Promise(resolve => proxy.on(event, _.once(resolve))).timeout(
				timeout,
				new AssertionError(
					errorMessage
						? errorMessage
						: `No event "${event}" received for ${timeout} milliseconds`
				)
			);
		}
						...
						_.once(
							reject(
								new AssertionError(
									errorMessage
										? errorMessage
										: `Unexpected event "${event}" received`
								)
							)
						);
					});
				...
				(proxy1, proxy2, proxy3, proxy4) => {
					return Promise.all([
						...
						expectNoProxyEvent(
							proxy1,
							'testContextEvent',
							'"testContextEvent" received without context'
						),
						expectNoProxyEvent(
							proxy4,
							'testContextEvent',
							'"testContextEvent" received for wrong context'
						),
