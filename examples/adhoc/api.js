...
const service = new Service('1.0.0', 'test-api', (methodName, options) => {
...
const testObj = {
	...
	echo(a) {
		console.log(a);
		const RenderMode = service.type('RenderMode').struct;
...
module.exports = (function() {
	service
		...
		.define(
			{ name: 'TestDefaultArray', params: [{ name: 'p', type: 'DefaultArray' }] },
			function(p) {
				console.log(p);
			}
		)
		.define({ name: 'TestUrl', params: [{ name: 'u', type: 'url' }], returns: 'url' }, function(
			u
		) {
			console.log(u);
			return u.format();
		...
		.define({
			name: 'testMe2',
			params: [{ name: 'a', type: 'string', description: 'A simple string parameter.' }],
			returns: 'string',
			description: 'A sample method.',
		})
		.define({
			name: 'echo',
			params: [{ name: 'a', type: 'RenderOptions' }],
		...
		.define(
			{
				name: 'echoStringAsBuffer',
				params: [{ name: 'theString', type: 'string' }],
				returns: 'binary',
			},
			function(theString) {
		...
		.define(
			{ name: 'getBufferSize', params: [{ name: 'buffer', type: 'binary' }], returns: 'int' },
			function(buffer) {
	...
	service.define(
		...
		function(required, p1, p2) {
			console.log('optionalArgs called with', arguments);
		}
	);
	service
		.define(
