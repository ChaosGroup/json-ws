		...
		for (const eventContextMapEntry of this.eventContextMap.entries()) {
			...
			if (eventContextSet.has(context)) {
				...
				if (eventContextSet.size === 0) {
					...
					if (eventListener) {
						const eventName = eventContextMapKey.substr(
							eventContextMapKey.indexOf(':') + 1
						);
					...
					for (const eventContext of eventContextSet) {
						...
						if (typeof matchContextParams === 'object') {
							...
							for (const contextKey in matchContextParams) {
								if (
									matchContextParams.hasOwnProperty(contextKey) &&
									eventContextParams[contextKey] !==
										matchContextParams[contextKey]
								) {
						...
						this.sendMessage(
							jsonrpc.response(eventName, null, eventData),
							eventContext
						);
	...
	call(service, request, context) {
		...
		try {
			...
			if (
				...
				(methodInfo.async || methodInfo.returns)
			) {
				// this is a Promise
				returnValue
					.then(function(_returnValue) {
