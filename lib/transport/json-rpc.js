...
const jsonrpc = (module.exports = {
	...
	validateRequest: function(req, api) {
		...
		if (Array.isArray(req.params)) {
			if (
				req.params.length < requiredParamsCount ||
				req.params.length > methodInfoParams.length
			) {
				return jsonrpc.error(
					-32602,
					'Invalid parameters',
					AsServiceError(
						'The specified number of parameters does not match the method signature',
						name
					)
				);
			}
			...
			for (let i = 0; i < methodInfoParams.length; i++) {
				if (
					i < requiredParamsCount &&
					!req.params.hasOwnProperty(methodInfoParams[i].name)
				) {
					return jsonrpc.error(
						-32602,
						'Invalid parameters',
						AsServiceError(
							'The specified parameters do not match the method signature',
							name
						)
					);
				}
