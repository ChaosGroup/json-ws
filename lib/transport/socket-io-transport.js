	...
	attachEvents(socket) {
		...
		socket.on('rpc.sio.setConnectionContext', ({validationParams = {}, serviceName, serviceVersion}, cb) => {
			if (!(serviceName && serviceVersion)) {
				socket.send(jsonrpc.response(
					null,
					jsonrpc.error(-32000, 'Bad Request', "No 'serviceName' or 'serviceVersion' provided to socket.io implementation.")));
				return;
			}
		...
		socket.on('message', message => {
			if (!connectionContext.service) {
				socket.send(jsonrpc.response(
					null,
					jsonrpc.error(-32000, 'Bad Request', 'Connection context is not set.')));
				return;
			}
