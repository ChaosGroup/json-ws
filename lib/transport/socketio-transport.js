...
class SocketIOTransport extends BaseTransport {
	...
	attachEvents(socket) {
		...
		connectionContext.data = connectionContext.data;
		connectionContext.params = null;

		this.onConnect(socket);

		socket.on('rpc.sio.setContext', ({validationParams, serviceName, serviceVersion}, cb) => {
			// TODO: validate the existence of serviceName and serviceVersion
		...
		socket.on('message', (message, cb) => {
			if (!connectionContext.service) {
				// TODO: Report meaningful error, maybe also to the client?
				console.error('No context set yet', message);
				return;
			}
