...
Service.prototype.define = function (options, fn) {
	...
	if (methodInfo.fn == undefined || fn) {
		if (!fn || typeof fn !== 'function') {
			methodInfo.fn = options["this"]
				? (function() {
					...
					if (typeof remappedFn !== 'function') {
						...
						return (internals.isGeneratorFunction(remappedFn) ? cowrap(remappedFn) : remappedFn).bind(self);
					} else {
			...
			methodInfo.fn = (internals.isGeneratorFunction(fn) ? cowrap(fn) : fn).bind(methodInfo["this"]);
		} else {
...
Service.prototype.import = function (importSite, whiteList) {
	...
	function forward(typeName, typeInfo) {
		...
		try {
			...
			// TODO throw error up
			// self.trace.error('Type importer', null, typeInfoParseError);
