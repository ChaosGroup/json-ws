...
const internalTypes = (() => {
	...
	const converters = {
		...
		streamConverter(value, out) {
			/*stream*/
			// TODO add conversion from/to buffers
			if (out) {
				if (value && value instanceof stream.Readable) {
					return value;
				}
				throw new Error('Readable stream expected.');
			}
			throw new Error('Input streams are not supported');
		},
		errorConverter(value) {
			/* error */
			if (value === null || value === undefined) {
				return value;
			} else if (
				value instanceof Error ||
				/Error$/.test(value.name) ||
	...
	return new Map([
		...
		['stream', converters.streamConverter],
