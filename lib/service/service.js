...
class Service extends EventEmitter {
	...
	define(options, fn) {
		...
		if (options.returns) {
			...
			if (!options.returns || (!types.isInternal(options.returns) && this.typeMap[options.returns]) === undefined) {
	...
	type(typeName, typeDef, description, isEnum) {
		...
		function typeConverter(value, out) {
			if (isEnum) {
				...
				if (!Object.keys(typeStruct).some(enumKey => {
	...
	defineAll(obj, methodNames) {
		...
		Object.getOwnPropertyNames(obj.constructor.prototype).forEach(ownProperty => {
			if (systemStuff.indexOf(ownProperty) > -1 || obj.hasOwnProperty(ownProperty) || checkSkipProperty(obj[ownProperty], ownProperty)) {
				return;
			}
