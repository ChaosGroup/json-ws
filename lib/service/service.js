...
class Service extends EventEmitter {
	...
	setRequestValidator(requestValidatorFn) {
		this.requestValidator = requestValidatorFn;
	}
	...
	_parseDefineParams(params) {
		...
		(params || []).forEach(function (param) {
			...
			if (param.type) {
				...
				if (!types.isInternal(param.type) && !self.typeMap[param.type]) {
	...
	define(options, fn) {
		...
		var requiresCallback = options.callback !== undefined ? options.callback : methodInfo.callback;
			...
			if (!fn || typeof fn !== 'function') {
				methodInfo.fn = options["this"]
					...
					if (typeof remappedFn !== 'function') {
						...
						return co.wrap(remappedFn).bind(self);
					} else {
	...
	type(typeName, typeDef, description, isEnum) {
		...
		var typeStruct = isEnum ? typeDef : types.typeStructFromDef(typeName, typeDef, function (typeToCheck) {
	...
	defineAll(obj, methodNames) {
		...
		var systemStuff = Object.getOwnPropertyNames({}.constructor.prototype);
...
module.exports = function createService(version, name) {
	return new Service(version, name);
};
