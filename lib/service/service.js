...
class Service extends EventEmitter {
		...
		(params || []).forEach(param => {
			...
			if (param.type) {
				...
				if (param.type === 'stream') {
					throw new Error('Input streams are not supported');
				}
	...
	define(options, fn) {
		if (options && options.event) {
			throw new Error(
				'Registering events using the define method is obsolete. Please use the event method instead.'
			);
		}
		...
		if (options.returns) {
			...
			if (
				!options.returns ||
				(!types.isInternal(options.returns) && this.typeMap[options.returns]) === undefined
			) {
				throw new Error(
					'Undefined return type for method ' + methodName + ': ' + options.returns
				);
			}
		}
		...
		methodInfo.description =
			options.description || methodInfo.description || types.missingDocumentation;
		methodInfo.examples = methodInfo.examples || {};

		const requiresCallback =
			options.callback !== undefined ? options.callback : methodInfo.callback;
	...
	type(typeName, typeDef, description, isEnum) {
		...
		if (typeof self.typeMap[typeName] === 'object') {
			throw new Error('Types cannot be overriden.');
		}

		const typeStruct = isEnum
			? typeDef
			: types.typeStructFromDef(typeName, typeDef, function(typeToCheck) {
					return self.typeMap[typeToCheck] !== undefined;
			  });
		...
		function typeConverter(value, out) {
			if (isEnum) {
				if (typeof value !== 'number' && typeof value !== 'string') {
					throw new Error(
						'Invalid value for enum ' +
							typeName +
							' - only numbers or strings are allowed.'
					);
				}
				let returnValue = value;
				if (
					!Object.keys(typeStruct).some(enumKey => {
						if (value === enumKey || typeStruct[enumKey] === value) {
							// Always output enum members as their int code
							// Otherwise validate if the value is a valid enum member and return as-is
							//returnValue = out ? typeStruct[enumKey] : value;
							returnValue = typeStruct[enumKey];
							return true;
						}
						return false;
					})
				...
				// Workaround to optionally return the old behaviour
				if (typeof returnValue === 'number' && Service._useStringEnums) {
					Object.keys(typeStruct).forEach(enumKey => {
						if (typeStruct[enumKey] === returnValue) {
							returnValue = enumKey;
						}
					});
				}

				return returnValue; // typeof value === 'number' ? value : typeStruct[value];
			}
	...
	event(eventName, eventInfo) {
		...
		if (!eventInfo && this.eventMap[eventName]) {
			// TODO: Check where this is used and remove it.
			// Currently it makes the function return type inconsistent.
		...
		if (eventInfo.type) {
			...
			if (
				!eventInfo.type ||
				(!types.isInternal(eventInfo.type) && !this.typeMap[eventInfo.type])
			) {
		...
		this.eventMap[eventName] = {
			...
			isArray: Boolean(eventInfo.isArray),
		};
	...
	defineAll(obj, methodNames) {
		...
		function checkSkipProperty(prop, name) {
			return (
				...
				name.substr(0, 1) === '_' ||
				(methodNames && methodNames.indexOf(name) === -1)
			);
		}
		...
		Object.getOwnPropertyNames(obj.constructor.prototype).forEach(ownProperty => {
			if (
				systemStuff.indexOf(ownProperty) > -1 ||
				obj.hasOwnProperty(ownProperty) ||
				checkSkipProperty(obj[ownProperty], ownProperty)
			) {
				return;
			}
			this.define(ownProperty, obj[ownProperty]);
		});

		if (EventEmitter.isPrototypeOf(obj.constructor)) {
			// listen for registered events on obj
			Object.keys(this.eventMap).forEach(eventName => {
				obj.on(eventName, this.emit.bind(this, eventName));
			});
		}
	 ...
	 * @returns {Service} The current API instance.
	 */
	import(importSite, whiteList, _level) {
		_level = _level || 1;
		const self = this;
		let defs;
		...
		const isPlaceholder = typeMapInfo => typeof typeMapInfo === 'number';
		...
		function forward(typeName, typeInfo) {
			...
			for (const propertyName in typeInfo) {
				...
				if (types.isInternal(propertyInfo.type) || isPlaceholder(typeMapInfo)) {
				...
				if (typeDeps.indexOf(propertyInfo.type) === -1) {
					...
					if (self.typeMap[propertyInfo.type] === undefined) {
						self.typeMap[propertyInfo.type] = _level;
					}
				}
			}
		}
		...
		function prepass(defs, level) {
			Object.keys(defs).forEach(function(key) {
				if (key === 'events' || key === 'methods') {
					Object.keys(defs[key]).forEach(function(name) {
						lists[key].push({
							name: name,
							group: currentGroup,
							data: defs[key][name],
					...
					importList.forEach(function(importItem) {
						if (typeof importItem === 'object') {
							...
							self.import(importItem.file, whiteList, _level + 1);
						} else {
							// Assume this is a file path
							self.import(importItem, null, _level + 1);
						}
					});
				} else {
		...
		function importTypeWithDependencies(chain, item) {
			...
			applyGroup(item.group, function() {
				...
				(deps[typeName] || []).forEach(function(referencedType) {
					if (isPlaceholder(self.typeMap[referencedType])) {
						if (!lists.types[referencedType]) {
							throw new Error(
								'Could not find a declaration for the following type: ' +
									referencedType
							);
						}
				...
				if (isPlaceholder(self.typeMap[typeName]) || !self.typeMap[typeName]) {
					// Don't delete the placeholder to allow definition of self-referencing recursive structures
					// For example, a directory tree
					// delete self.typeMap[typeName];
					self[item.isEnum ? 'enum' : 'type'](
						typeName,
						typeInfo.struct,
		...
		lists.types
			.filter(filterWhiteList.bind(null, 'enum:'))
			.filter(item => item.isEnum)
			.forEach(function(item) {
				applyGroup(item.group, function() {
					if (!self.typeMap[item.name] || isPlaceholder(self.typeMap[item.name])) {
		...
		lists.types
			.filter(filterWhiteList.bind(null, 'type:'))
			.filter(item => !item.isEnum)
			.forEach(importTypeWithDependencies.bind(null, []));

		// do this check only on level "0" to prevent false negatives in nested imports:
		if (_level === 1) {
			Object.keys(self.typeMap).forEach(function(typeName) {
				if (isPlaceholder(self.typeMap[typeName])) {
					if (
						self.typeMap[typeName] === 1 &&
						(filterWhiteList('type:', typeName) || filterWhiteList('enum:', typeName))
					) {
	...
	examples(importSite) {
		...
		while (lineEnd > -1) {
			...
			if (name) {
				// inside an example or a snippet
				if (line.indexOf('{example}') > -1) {
					...
					name = null;
				} else if (line.indexOf('{snippet}') > -1) {
					// end of the snippet
				...
				if (index !== -1) {
					// beginning of an example or a snippet
	...
	static setUseStringEnums(useStringEnums) {
		Service._useStringEnums = useStringEnums;
	}
}
