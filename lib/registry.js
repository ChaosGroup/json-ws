...
class ServiceRegistry {
	...
	_renderMetadataPage(service, servicePathPrefix, req, res) {
		...
		if (req.query.proxy !== undefined) {
			...
			ejs.renderFile(__dirname + '/../templates/viewer.ejs', {
				...
				snippets,
				code,
				client: fs.readFileSync(__dirname + '../lib/client/index.js')
	...
	_routeHandler(req, res, next) {
		...
		} else if (requestUrl == '/client' || requestUrl == '/index.js') {
			fs.readFile(__dirname + '../lib/index.js', {encoding: 'utf-8'}, function (err, text) {
