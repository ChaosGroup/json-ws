...
function getBase64Images(imagePaths) {
	const base64Images = {};
	for (const imagePath of imagePaths) {
		const imageName = imagePath.split('.')[0];
		base64Images[imageName] = fs.readFileSync(path.join(__dirname, '../static', imagePath), 'base64');
	}
...
class ServiceRegistry {
	...
	_renderMetadataPage(service, servicePathPrefix, req, res) {
		...
		if (req.query.proxy !== undefined) {
			...
			ejs.renderFile(__dirname + '/../templates/viewer.ejs', {
				...
				url: `${this.rootPath}${servicePathPrefix}`,
				examples,
				snippets,
				code,
			...
			fs.readdir(__dirname + '/../proxies/', (err, files) => {
				...
				const imagesPaths = ['event.png', 'type.png', 'snippet.png', 'method.png'];
				...
				ejs.renderFile(
					__dirname + '/../templates/metadata.ejs',
					{
						...
						proxies,
						root: this.rootPath,
						images: getBase64Images(imagesPaths)
	...
	_routeHandler(req, res, next) {
		...
		if (requestUrl === '/') {
			...
			if (serviceInstance && req.method.toUpperCase() == 'GET') {
				this._renderMetadataPage(serviceInstance, requestUrl, req, res);
			} else {
				next();
			}
		}
