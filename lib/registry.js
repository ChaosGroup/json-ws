...
class ServiceRegistry {
	constructor(rootPath, httpServer, expressApp) {
	...
	attachExpressRouter() {
		this.expressApp.use(this.rootPath, this.router);
	}

	addService(service) {
		const rootPrefix = `/${service.name}/${service.version}`;
	...
	_routeHandler(req, res, next) {
		...
		if (requestUrl === '/') {
			...
			if (serviceInstance && req.method.toUpperCase() == 'GET') {
				this.renderMetadataPage(serviceInstance, req, res);
			} else {
				next();
			}
		}
...
module.exports = function(rootPath, httpServer, expressApp) {
	let registry = registries.get(rootPath);
	if (!registry) {
		registry = new ServiceRegistry(rootPath, httpServer, expressApp);
